  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.data.insertedId) {
        ...
        // 🔻 Reduce worker count instantly without reload
        setTask((prev) => ({ ...prev, worker: prev.worker - 1 }));
    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
      <div className="bg-white shadow-xl rounded-2xl p-6 md:p-10 border border-gray-100">
        ...
        <form onSubmit={handleSubmit} className="mt-10">
          ...
          <button
            type="submit"
            disabled={task.worker < 1}
            className={`mt-6 px-6 py-3 rounded-lg text-white font-medium transition-all shadow-sm ${
              task.worker < 1
                ? "bg-gray-400 cursor-not-allowed"
                : "bg-blue-600 hover:bg-blue-700"
            }`}
          >
            {task.worker < 1 ? "Task already completed" : "✅ Submit Task"}
          </button>
        </form>
      </div>
    </div>
